...
//        log.debug("Game tick: {}", client.getTickCount());
        ...
        if (!Objects.equals(npcDialogState, lastNpcDialogState)) {
            log.debug("{} npc dialog changed: {} previous: {} (game tick: {})", client.getTickCount(), npcDialogState, lastNpcDialogState);
    ...
    private void optionSelected(NpcDialogState state, String option) {
        ...
        if (state.type == NpcDialogState.NpcDialogType.OPTIONS) {
            log.debug("{} option selected: \"{}\" {}", client.getTickCount(), option, state);
        } else {
            log.debug("{} clicked here to continue: {}", client.getTickCount(), state);
        }
